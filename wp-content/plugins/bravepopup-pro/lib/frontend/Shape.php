   ...
   class BravePop_Element_Shape {
      ...
      public function render( ) { 
         ...
         if($shapeType === 'icon' && isset($this->data->icon->body)){
            $iconHTML = '<div class="brave_element_shape-icon"><svg viewBox="0 0 '.$icon->width.' '.$icon->height.'" width="1em" height="1em" preserveAspectRatio="xMidYMid meet" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">'.str_replace('currentColor', $shapeData->fillColor ,$icon->body).'</svg></div>';
         }
         if($shapeType === 'icon' && !isset($this->data->icon->body)){
